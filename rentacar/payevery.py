'''
PAY EVERY
Divided with payday, work EVERY day, and add pay to active contracts renta_price / 30. Made for transition to daily paymenе
...
def start_payevery(db: client):
    ...
    for contract in contracts:
        ...
        if contract['nickname'] not in [task['nickname'] for task in tasks if task['name_task'] == 'PayDay' and task['status']] and contract['last_saldo'] < -contract['renta_price']:
...
def start_payevery2(db: client):
    ...
    for contract in contracts:
        if contract['nickname'] not in [task['nickname'] for task in tasks if task['name_pay'] == 'PayDay' and task['status']] and contract['last_saldo'] < -contract['renta_price'] / 30:
            create_task(db, contract)
...
def create_payevery(db: client, contract: dict, odometer: int):
    ...
    """
    print(f'write payevery - nickname: {contract["nickname"]}')
    if '--read-only' not in argv:
