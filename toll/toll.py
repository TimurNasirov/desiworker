...
from str_config import TOLL_USERNAME_ID, TOLL_PASSWORD_NAME, TOLL_LOGIN_BUTTON_SELECTOR, TOLL_CSV_XPATH, TOLL_FILENAME, USER
logdata = Log('toll.py')
print = logdata.print
...
def start_toll(db: client):
        ...
        for row in readed_data:
            if row[0].strftime('%m.%d.%Y') in [(dt.now() - timedelta(days=i)).strftime('%m.%d.%Y') for i in range(4)]:
                tolls.append({
                    ...
                    'name_pay': 'Toll',
                    'id': int(row[1]),
                    'comment': 'location: ' + row[2] + ', plate: ' + row[4][row[4].rfind(' ') + 1:len(row[4])] + ', type: ' + row[5],
                    'sum': row[8],
                    'expense': True,
                    'income': False,
                    'owner': False,
    ...
    for toll in tolls:
        if toll['id'] not in exists_tolls:
            ...
            plate = toll['plate']
            toll.pop('plate')
            if car['nickname'] != None: toll['nickname'] = car['nickname']
            toll['ContractName'] = get_contract(db, toll['nickname'], check_active=False)['ContractName']
            print(f'write toll {toll["id"]}, nickname: {toll["nickname"] if has_key(toll, "nickname") else "-"}.')
            if '--read-only' not in argv:
                db.collection('Pay_contract').add(toll)
                toll.pop('ContractName')
                toll['paid'] = True
                toll['plate'] = plate
